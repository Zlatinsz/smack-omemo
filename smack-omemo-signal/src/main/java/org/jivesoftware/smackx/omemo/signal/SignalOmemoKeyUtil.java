 ...
 * @author Paul Schaub
 */
class SignalOmemoKeyUtil extends KeyUtil<IdentityKeyPair, IdentityKey, PreKeyRecord, SignedPreKeyRecord,
    ...
    public String getFingerprint(IdentityKey identityKey) {
        ...
        return fp.replace("(byte)0x", "").replace(",", "").replace(" ", "").substring(2);
    }

    @Override
    public SignalProtocolAddress omemoContactAsAddress(OmemoDevice contact) {
        return new SignalProtocolAddress(contact.getJid().asBareJid().toString(), contact.getDeviceId());
    }

    @Override
    public OmemoDevice addressAsOmemoContact(SignalProtocolAddress address) throws XmppStringprepException {
